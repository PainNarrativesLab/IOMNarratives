...
class ConditionsService(IOMService):
	...
	def make_vectors(self):
		...
		for c in self.conditions:
			...
			try:
				rtemp = []
				[rtemp.append(rxc['respondentID']) for rxc in self.respondentsXconditions if str(rxc['conditionName']) == condition]
			...
			try:
				qtemp = []
				[qtemp.append(qxc['quoteID']) for qxc in self.quotesXconditions if str(qxc['conditionName']) == condition]
			...
			try:
				ptemp = []
				[ptemp.append(pxc['respondentID']) for pxc in self.patientsXconditions if str(pxc['conditionName']) == condition]
