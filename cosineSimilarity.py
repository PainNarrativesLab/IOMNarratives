...
class cosine_simiarlity_analyzer():
	def __init__(self, list_of_texts, data):
		...
		self.distances = {}
		for (title1, link1) in self.td_matrix.keys():
			self.distances[(title1, link1)] = {}
			(max_score, most_similar) = (0.0, (None, None))
			for (title2, link2) in self.td_matrix.keys():
				...
				self.similarities.append(dict(rootID=title1, friendID=title2, tfidf_score=nltk.cluster.util.cosine_distance(v1, v2)))
				if link1 == link2:
					continue
